...
model1 = Dense(200, activation='sigmoid', name='Indian_Pines_Output_1')(model1)

combinedoutput= Dense(1024,activation='relu', name= 'Classifier_Input_1')(model1)
...
combinedoutput= Dense(16,activation='softmax', name='Classifier_Output_1')(combinedoutput)


model = Model(inputs=modelinput1, outputs=combinedoutput)
sgd = optimizers.SGD(lr=0.0001, momentum=0.9, decay=1e-6)
model.compile(optimizer=sgd, loss='categorical_crossentropy', metrics=['accuracy'])

checkpoint = ModelCheckpoint('/home/SharedData/Avinandan/DarrellHallucination/streamone.h5', monitor='val_acc', verbose=1,save_best_only=True, mode='max')
callback_list = [checkpoint]

model.fit(
    Xtrain_one, Ytrain, callbacks=callback_list,
    validation_data=(Xtest_one,Ytest),
