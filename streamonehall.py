...
streamonefeatures = Model(inputs=streamone.get_layer('Indian_Pines_Input_1').input,
                       outputs=streamone.get_layer('Indian_Pines_Output_1').output)
...
model1 = Dense(200, activation='sigmoid', name='Indian_Pines_Output_1_Hallucinated')(model1)
model1hall = model1
...
for layer in streamone.layers[18:]:
    model1 = layer(model1)


original = streamonefeatures(modelinput1)

model = Model(inputs=[modelinput2, modelinput1], outputs=model1)
...
model.compile(optimizer=sgd,
              loss=custom_loss(hall=model1hall, orig=original),
              metrics=['accuracy'])

model_json = model.to_json()
with open("/home/SharedData/Avinandan/DarrellHallucination/streamonehall.json", "w") as json_file:
    json_file.write(model_json)


checkpoint = ModelCheckpoint('/home/SharedData/Avinandan/DarrellHallucination/streamonehall.hdf5', monitor='val_acc', verbose=1,save_best_only=True, mode='max')
callback_list = [checkpoint]

model.fit(
    [Xtrain_two,Xtrain_one], Ytrain, callbacks=callback_list,
    validation_data=([Xtest_two,Xtest_one],Ytest),
